        ...
        this.chiefComplaintService.getChiefComplaintKeywords(this.chiefComplaintId)
            .then(keywords => {
                ...
                if (this.newKeywordsToAdd && this.newKeywordsToAdd.length) {
                    const notAddedNewKeywords = this.newKeywordsToAdd
                        .filter(newKeyword => {
                            if (!keywords.length)
                                return true;

                            const existingKeyword =
                                this.keywords.find(k => KeywordEqualityComparerService.areKeywordsEqual(k, newKeyword));

                            return !existingKeyword;
                        });

                    this.keywords =
                        this.keywords.concat(notAddedNewKeywords);
                }
            })
