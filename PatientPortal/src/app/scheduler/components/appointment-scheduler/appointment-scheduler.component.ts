...
export class AppointmentSchedulerComponent implements OnInit, OnDestroy {
    ...
    return {
      ...
      editorOptions: {
        items: this.selectableListService
          .getSelectableListValuesFromComponent(this, "appointmentStatus")
      }
    }
  }
  ...
  private updatePatientPreviousAdmissionsState(startDate, patientId, patientAppointmentKey) {
    this.appointmentService.getPatientPreviousVisits(patientId, startDate)
      .then(result => {
        this.previousAdmissionsState[patientAppointmentKey] = !!result.length;
      });
  }
      ...
      return nativeLoadFunc.call(store, loadOptions)
        .then(result => {
          result.forEach(appointmentGridItem => {
            ...
            const patientAppointmentKey =
              this.getPatientAppointmentKey(appointmentGridItem.patientId, appointmentGridItem.id);

            this.previousAdmissionsState[patientAppointmentKey] = false;

            this.updatePatientPreviousAdmissionsState(appointmentGridItem.startDate,
              appointmentGridItem.patientId, patientAppointmentKey);
          });
          return result;
        });
    };
  ...
  private initSelectableLists(companyId: string): void {
    ...
    const appointmentStatusListConfig =
      new SelectableListConfig(companyId, PredefinedSelectableListsNames.appointmentStatus);

    this.selectableListService.setSelectableListsValuesToComponent([appointmentStatusListConfig], this)
      .then(() => {
